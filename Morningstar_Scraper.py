...
browser.get('http://tools.morningstar.nl/nl/etfquickrank/default.aspx?Site=NL&Universe=ETALL%24%24ALL&LanguageId=nl-NL')
...
browser.find_element(By.LINK_TEXT, "Professionele belegger").click()
...
select.select_by_visible_text("500 per pagina")
...
while keep_going:
    ...
    try:
        if browser.find_element_by_xpath("(//A[@class='pager_disabled disabled'][text()='Volgende'])"):
            keep_going = False

    except:
        v = browser.find_element(By.LINK_TEXT, "Volgende")
        browser.execute_script('arguments[0].click()', v)
        time.sleep(randint(5, 8))

# Append data to Pandas Dataframe

df = pd.DataFrame({'name': col1, 'link': col2, 'TER': col3, 'close': col4, 'stars': col5})
